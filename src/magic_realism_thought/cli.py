    ...
    if trace.decision_landscape:
        ...
        for decision in trace.decision_landscape[-max_rows:]:
            ...
            lines.append(
                ...
                f"margin={margin}; deferred={deferred}"
            )
...
def format_rpm_markdown(trace: RPMTrace) -> str:
    ...
    if trace.decision_landscape:
        ...
        lines.append("")
        lines.append("| decision | stage | accepted | score | margin | rejected/repair candidates | deferred |")
        lines.append("|---|---:|---|---:|---:|---|---|")
        for decision in trace.decision_landscape:
            ...
            lines.append(
                ...
                f"{score} | {margin} | {', '.join(rejected[:5]) or '-'} | {deferred} |"
            )
        lines.append("")
...
def compute_reward_surface_audit(run: ChainRun) -> RewardSurfaceAudit:
    ...
    saturated = [
        f"{axis}:mean={mean(values):.3f}"
        for axis, values in sorted(axis_values.items())
        if len(values) >= 3 and mean(values) >= 0.90
    ]
    ...
    for decision in run.rpm_trace.decision_landscape:
        ...
        if candidate_count >= 2 and decision.selection_margin is not None and decision.selection_margin < 0.035:
            low_margin.append(f"{decision.stage_index}:{decision.stage_name} margin={decision.selection_margin:.4f}")
    ...
    for step in run.steps:
        ...
        if closest >= 0.92:
            low_diversity.append(f"{step.index}:{step.name} closest_rejected_similarity={closest:.3f}")
